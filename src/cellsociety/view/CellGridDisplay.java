...
public class CellGridDisplay extends ChangeableDisplay {
   ...
   *
   * @param controller will be used to get things like grid shape and a list of cells. I don't love
   *                   the idea of making myController an instance variable here but it's convenient
   *                   -Keith
   */
  public CellGridDisplay(Controller controller) {
    ...
    myCornerGenerator = new HexagonCellCornerLocationGenerator(controller.getGridShape()[0],
        controller.getGridShape()[1]);
  }
  ...
  public Node createGridDisplay() {
    ...
    myGroup.getChildren().add(new Rectangle(0, 0, myViewResourceHandler.simulationWidth(),
        myViewResourceHandler.simulationWidth()));
    myCornerGenerator = new RectangleCellCornerLocationGenerator(myController.getGridShape()[0],
  ...
  private CellDisplay makeACellDisplay(ImmutableCell cell, double widthPerCell,
      double heightPerCell) {
    ...
    myController.addCellObserver(cell.getiIndex(), cell.getjIndex(),
        cellState -> newDisplay.changeState(cellState));
    return newDisplay;
  }
   ...
   *
   * @param newShape is a String like "Triangle" or "Rectangle" that corresponds to a valid cell
   *                 shape
   */
  public void changeCellShapes(String newShape) {
    myCornerGenerator = myViewResourceHandler.getCornerLocationGenerator(newShape,
        myController.getGridShape()[0],
        myController.getGridShape()[1]);
    try {
