...
public class HistogramDisplay extends ChangeableDisplay {
   ...
   * @param stateToCount maps each state to the number of cells in that state. It must contain a key
   *                     for each possible state!
   */
  public HistogramDisplay(int numCells, Map<Integer, Integer> stateToCount,
      LanguageResourceHandler l) {
    super(l);
    initializeSettings();
  ...
  private void initializeSettings() {
    ...
    maxBarHeight = viewSettings.getViewSettingValueFromKey(
        ViewResourceHandler.MAX_HISTOGRAM_BAR_HEIGHT_KEY);
  ...
  private void initializeRectangles(Map<Integer, Integer> stateToCount) {
    ...
    int startX = viewSettings.getViewSettingValueFromKey(
        ViewResourceHandler.DIST_BTWN_HISTOGRAM_BARS_KEY);
    int barWidth =
        viewSettings.getViewSettingValueFromKey(ViewResourceHandler.HISTOGRAM_WIDTH_KEY) / (2
            * stateToCount.size());
    for (Integer state : stateToCount.keySet()) {
      ...
      startX = startX + barWidth + viewSettings.getViewSettingValueFromKey(
          ViewResourceHandler.DIST_BTWN_HISTOGRAM_BARS_KEY);
    }
  ...
  public Node createHistogramDisplay() {
    ...
    Rectangle backGround = new Rectangle(0, 0,
        viewSettings.getViewSettingValueFromKey(ViewResourceHandler.HISTOGRAM_WIDTH_KEY),
    ...
    container.getChildren().add(new Line(0, zeroBarY,
        viewSettings.getViewSettingValueFromKey(ViewResourceHandler.HISTOGRAM_WIDTH_KEY),
        zeroBarY));
  ...
  /**
   * use this method to change the height of a bar on the histogram, to reflect the number of cells
   * of a given state
   *
