 ...
 * This new visualization involves displaying each state, the image/display for that state, and the change in the last
 * step in the number of cells in that state
...
public class InfoDisplay extends ChangeableDisplay{
   ...
   * Note: HistogramDisplay has a very similar method, but I didn't think it was worth it to work this into some
   * sort of inheritance hierarchy, as it would only affect one method in these two classes.
   * If there were like 10 different display options then I would go for it, but it seems like overkill here. -Keith
