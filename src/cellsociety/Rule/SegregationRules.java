...
public class SegregationRules extends Rules {
  ...
  public void setState() {
    ...
    if ((satisfaction) <= satisfactionThreshold && cell.getCurrentState() != 0) {
      if (cell.getCurrentState() != 0 && cell.numOfStateNeighbors(0) > 0) {
        moveCell();
      } else if (cell.getCurrentState() != 0 && cell.numOfStateNeighbors(0) == 0) {
        moveRandom(cell);
      } else {
  ...
  /**
   * @param cell Moves the current cell into an open cell and sets the current cell to state 0
   */
   ...
   * @return a random cell with current and future state 0 searches recursively through the
   * neighbors of a cell's neighbors
   */
  public Cell findOpenCell(Cell c) {
    ...
    for (Cell cell2 : neighborsNeighboringCells) {
      if (cell2.getFutureState() == 0 && cell2.getCurrentState() == 0) {
        return cell2;
      }
    }
    return findOpenCell(
        neighborsNeighboringCells.get(chooseRandomIndex(neighborsNeighboringCells.size())));
  }
  ...
  public int numberOfNeighborsWithOtherState() {
    if (cell.getCurrentState() == 1) {
      return cell.numOfStateNeighbors(2);
    }
