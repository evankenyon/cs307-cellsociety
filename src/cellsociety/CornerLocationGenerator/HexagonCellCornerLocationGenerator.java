...
public class HexagonCellCornerLocationGenerator extends CornerLocationGenerator {
   ...
   * @param i
   * @param j
   * @return a list of the corners for each individual hexagon based on its index and the size of
  ...
  public List<CornerLocation> generateCorners(int i, int j) {
    ...
    CornerLocation bottomLeft = new CornerLocation(center.getX_pos() - .5 * width,
        center.getY_pos() + size - .5 * width * Math.cos(Math.toRadians(30)));
    CornerLocation bottomRight = new CornerLocation(center.getX_pos() + .5 * width,
        center.getY_pos() + size - .5 * width * Math.cos(Math.toRadians(30)));
    CornerLocation topLeft = new CornerLocation(center.getX_pos() - .5 * width,
        center.getY_pos() - size + .5 * width * Math.cos(Math.toRadians(30)));
    CornerLocation topRight = new CornerLocation(center.getX_pos() + .5 * width,
        center.getY_pos() - size + .5 * width * Math.cos(Math.toRadians(30)));
    corners.add(top);
    corners.add(topLeft);
