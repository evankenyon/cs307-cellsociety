...
public class TriangleCellCornerLocationGenerator extends CornerLocationGenerator {
   ...
   * @param i
   * @param j
   * @return a list of the corners for each individual triangle based on its index and the size of
  ...
  public List<CornerLocation> generateCorners(int i, int j) {
    ...
    if (j % 2 == 0) {
      ...
      CornerLocation bottomLeft = new CornerLocation(center.getX_pos() - .5 * length,
          center.getY_pos() + .5 * height);
      CornerLocation bottomRight = new CornerLocation(center.getX_pos() + .5 * length,
          center.getY_pos() + .5 * height);
      corners.add(top);
      corners.add(bottomRight);
      corners.add(bottomLeft);
    } else {
      CornerLocation bottom = new CornerLocation(center.getX_pos(),
          center.getY_pos() + .5 * height);
      CornerLocation topLeft = new CornerLocation(center.getX_pos() - .5 * length,
          center.getY_pos() - .5 * height);
      CornerLocation topRight = new CornerLocation(center.getX_pos() + .5 * length,
          center.getY_pos() - .5 * height);
      corners.add(bottom);
      corners.add(topLeft);
